...
-   Loan Term Adjustment: The model can restructure the loan to shorten or extend the repayment years, thereby changing the scheduled monthly payment. If the loan is fixed and the payment increase exceeds the Penalty-Free Payment Increase Threshold, or if the term is extended (reducing the payment), it constitutes a broken contract and triggers the full wholesale swap rate penalty calculation plus the Flat Restructure Fee.
